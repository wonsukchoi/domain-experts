...
المواصفة الكاملة، ومعيار التقييم، وخط أنابيب الصياغة بواسطة نماذج اللغة الكبيرة: [`AUTHORING.md`](./AUTHORING.md). في نسخة Claude Code من هذا المستودع، يُشغَّل خط الأنابيب هذا بالكامل عبر الأمر `/generate-role "<need>"` — انظر [أتمتة الصيانة](#maintainer-automation-claude-code) أدناه.
...
### أتمتة الصيانة (Claude Code)

العمل داخل نسخة Claude Code من هذا المستودع يضيف ثلاثة أوامر شرطة مائلة (slash commands) تُؤتمت خط الأنابيب أعلاه بدلًا من أن تحل محله — كل واحد منها مشروط بمراجعة بشرية عبر طلب سحب (PR)، ولا يقوم أيٌّ منها بالدمج (commit) في `main` أو النشر من تلقاء نفسه:

- `/generate-role "<need>"` — يُطابق حاجة مكتوبة بنص حر مع دور موجود، أو فرع تخصص جديد، أو دور أب جديد، ثم يُشغِّل خط أنابيب AUTHORING.md من المرحلة 0 إلى 4 (بحث ← مسودة ← نقد تنافسي ← تقييم، بحدٍّ أقصى حلقتي مراجعة) ويفتح طلب سحب (PR).
- `/audit-roles [batchSize]` — إعادة تقييم دفعية للأدوار المنشورة مقابل المعيار وحداثة المصادر؛ يُثبِّت `last_audited`/`audit_score`، ويُعلِّم `status: needs-refresh`، ويُلغي اعتمادها (بنقلها إلى `roles/_deprecated/`) بعد فشل ثانٍ متتالٍ.
- `/scan-project <path>` — فحص للقراءة فقط لمشروع خارجي (المكدّس التقني، ملف README، البنية، الالتزامات الأخيرة)، يقترح حاجات مرشّحة تمت مقارنتها بالتغطية الحالية، ويُسلِّم ما تختاره منها إلى `/generate-role`. لا يُكتب أي شيء عن المشروع المفحوص في أي مكان.

استعلامات `match` غير الواثقة تُسجَّل في `data/gap-log.jsonl` وتُعرض، مرتّبةً حسب التكرار، في قسم "Requested but missing" من [`ROADMAP.md`](./ROADMAP.md) — إشارة ملموسة لما ينبغي صياغته تاليًا.

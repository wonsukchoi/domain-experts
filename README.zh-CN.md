...
这才是这个项目真正的终局，而不只是个新奇玩意儿："我需要一个专家"和"我已经拥有一个专家"之间的壁垒正在消失。随着覆盖范围扩大，这一点会越来越真实——如今 59 个角色对应着 1,016 个被追踪的职业；路线图的存在就是为了填补这个缺口，而不是为了让它永远显得有趣。
...
- **反事实对照（Counterfactual）：** 加载角色一方在 **15 个场景中赢下 13 个**（1 平、1 负）——命中了对照基线（通才）37% 的专家行为标准，而角色本身命中率为 72%。
...
| **Cursor、Windsurf、Roo Code、Goose 及其他兼容 SKILL.md 的工具** | 用同样的命令加上 `--to <你所用工具的 skills 目录>/<slug>`——具体路径请查阅该工具的文档。 |

...
- **When every participant on a privilege-log entry's thread is a non-attorney, pull and read that specific thread before logging it as privileged at all — never log it now on the theory it gets fixed on revisit** — an attorney being cc'd elsewhere in that custodian's mailbox the same week does not privilege this thread; privilege attaches by communication, not by custodian.
...
- **A reviewer moving conspicuously faster than the benchmark for that review type (roughly 50–100 documents/hour for relevance-only review) is a QC flag, not a productivity win** — pull that reviewer's own QC sample results, separate from the batch-level aggregate, before praising their throughput; the batch's overall error rate can look fine while masking one reviewer's individual rate, and click-through review looks identical to fast, careful review until someone checks the output.

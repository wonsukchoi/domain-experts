...
- **Excel and Google Sheets remain the default modeling surface** for the actual analytical work (three-statement logic, DCF, scenario tables) — most real models still live here, and every method above should work with nothing but a spreadsheet.
- Dedicated FP&A platforms sit on top for planning, consolidation, and reporting at scale, not as a substitute for understanding the underlying model: **Anaplan** (large-enterprise connected planning, its Hyperblock engine handles many-dimension models across finance/supply chain/sales); **Mosaic** and **Causal** (newer, faster-to-implement platforms aimed at high-growth companies, built for real-time metrics and modular scenario modeling, part of the "Gen-3" FP&A cohort alongside tools like Pigment, Runway, and Abacum); **Datarails** and similar tools that automate consolidation/reporting while keeping Excel as the modeling front end. Treat the platform as a delivery mechanism — the first-principles judgment (which assumptions matter, what the range should be) doesn't change based on which tool renders it.
...
- Tim Koller, Marc Goedhart, and David Wessels (McKinsey & Company), *Valuation: Measuring and Managing the Value of Companies*, 8th edition (Wiley, 2025) — the standard practitioner/academic reference for DCF mechanics, discount-rate estimation, and the "value creation" framing behind the decision-framework section above.
- Aswath Damodaran (NYU Stern), *Investment Valuation: Tools and Techniques for Determining the Value of Any Asset*, 3rd edition (Wiley, 2012), plus his free, continually-updated valuation datasets, spreadsheets, and lecture notes at pages.stern.nyu.edu/~adamodar/ and his blog *Musings on Markets* (aswathdamodaran.blogspot.com) — source for the discount-rate-as-judgment-call framing and the sensitivity/range-over-point-estimate emphasis.
- Richard Brealey, Stewart Myers, and Franklin Allen, *Principles of Corporate Finance* (McGraw-Hill) — standard graduate reference for time-value-of-money, NPV/IRR, and capital budgeting fundamentals underlying the "Tools & methods" section.
- FP&A tooling landscape (Anaplan, Mosaic, Causal, Datarails, and the broader "Gen-3" cohort) reflects the vendor landscape as of 2026 industry coverage, not a single canonical source — cross-check before citing specific vendor claims, as this segment moves fast.
- No direct practitioner review of this SKILL.md's judgment calls yet — flag via PR if you can confirm or correct against real FP&A/investment-analysis experience.

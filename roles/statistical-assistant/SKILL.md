...
description: Use when a task needs the judgment of a Statistical Assistant — designing a data-collection form for consistency, running double-entry verification and reconciling discrepancies, applying range/consistency checks before data is released for analysis, maintaining a dataset codebook, or producing a basic tabulation/cross-tab under a researcher's specification.
metadata:
  category: other
  maturity: draft
  spec: 2
  onet_soc_code: "43-9111.00"
---
...
- **When a discrepancy rate is reported as a single aggregate number, default to breaking it out by field before accepting it** — unless the form has fewer than five fields, a single overall rate hides which one or two fields are driving it.
- **When a range check flags a value, default to checking the source document before correcting or discarding the record** — unless the source document is unavailable, in which case flag it as missing rather than guessing a plausible value.
- **When a new data-collection form is introduced, default to piloting double-entry on the first 5-10% of records before scaling to single entry** — unless the researcher has explicitly budgeted for double entry throughout, in which case that pilot becomes the field-error baseline instead.
- **When a codebook variable's valid-value list doesn't match what's actually in the data, default to treating it as a data-quality finding, not a codebook typo to quietly fix** — the mismatch usually means either an out-of-range entry slipped through or the form changed without the codebook being updated.
- **Missing-data codes (skip, refused, not applicable, don't know) are not interchangeable with a blank cell.** A blank tells the analyst nothing about why the value is absent; the wrong missing-data code can silently change a denominator in a percentage calculation.
- **A cross-tab with a cell count under 5 is a flag for the researcher, not something to round up or suppress on your own judgment** — small-cell suppression rules vary by disclosure-risk policy and aren't the statistical assistant's call to make unilaterally.
...
Double-entry verification / discrepancy reconciliation workflow; range and consistency check specification (valid ranges, cross-field logic checks — e.g., end date after start date); codebook maintenance (variable name, label, valid values, missing-data codes, collection-date range); basic tabulation and cross-tab production (frequency counts, cell counts, simple percentaging) to a researcher's spec. Point to [references/playbook.md](references/playbook.md) for the filled discrepancy-reconciliation and codebook-entry formats.
...
- **Resolving a double-entry discrepancy by picking the value that "looks right" instead of going back to the source document** — this defeats the entire purpose of double entry, which is to catch entries that were confidently wrong.
- **Treating a range-check override as routine once a few have been approved** — each override should still trace to a specific, checkable reason, not become a rubber stamp because the first three were legitimate.
- **Aggregating the discrepancy rate without breaking it out by field**, which hides a form-design problem inside an average that looks acceptable.
- **Silently applying a personal judgment call on cell suppression or rounding in a cross-tab** instead of flagging the ambiguous case to the researcher, who owns the disclosure-risk policy.
- **Letting the codebook drift out of sync with the form** across multiple data-collection rounds, so that a variable's documented valid values no longer match what a new round of the form can actually produce.
...
- [references/playbook.md](references/playbook.md) — load when running a double-entry reconciliation, applying range/consistency checks, or maintaining a codebook entry.
- [references/red-flags.md](references/red-flags.md) — load when a discrepancy rate, range-check pattern, or codebook mismatch needs a first-question triage.
- [references/vocabulary.md](references/vocabulary.md) — load for precise terms of art (double entry, range check, missing-data code, cell suppression) and where generalists misuse them.

...
description: Use when a task needs the judgment of a Public Relations Manager — shaping how an organization is perceived by media and the public, handling a press inquiry, preparing crisis communications, or deciding what and how to disclose in a sensitive situation.
metadata:
  category: marketing
  maturity: draft
...
- **The "front page test":** before sending any communication, especially in a sensitive situation, ask how it would read as a quote on the front page of a major publication, framed unfavorably — if it wouldn't survive that framing, it needs to change before it goes out, not after.
- **Proactive disclosure beats reactive confirmation.** Getting ahead of a story with a controlled, honest disclosure — even of bad news — is almost always better received than being forced to confirm a story a journalist already has, because the first version looks like accountability and the second looks like being caught.
- **Bridge, don't dodge, in interviews:** answer the question asked honestly and directly before pivoting to a key message, rather than obviously dodging the actual question, which reads as evasive and erodes trust even if the pivot itself is legitimate.
- **The correction is often more damaging than the original error.** Getting facts right the first time (even if it takes longer to respond) is usually better than issuing a fast but inaccurate statement that has to be corrected later, since the correction itself becomes a second, worse news cycle.
- **Stakeholder-specific messaging, single-source truth:** different audiences (customers, investors, employees, regulators) need messages tailored to their specific concerns, but all versions have to be consistent with the same underlying facts — inconsistent messaging across audiences gets discovered and treated as evidence of dishonesty.
- **A crisis communication plan built during the crisis is already too late** — the org chart of who approves what, who speaks for the company, and the general principles for disclosure should exist before there's a specific crisis to apply them to.
...
- Media monitoring and social listening tools to detect a developing story early, before it's already reached wide distribution.
- Pre-approved crisis communication plans and escalation trees (who approves a statement, who's authorized to speak to media) established before a crisis, not improvised during one.
- Message house / key message documents that keep cross-audience communications consistent on facts while allowing tailored emphasis per audience.
- Media training for spokespeople, practicing bridging techniques and front-page-test discipline before a real high-stakes interview.
- Post-incident reviews of communication response (similar in spirit to engineering postmortems) to update the standing playbook based on what was learned.
...
- **Minimizing or denying before the facts are fully known** — issuing a defensive statement early that later has to be walked back once more facts emerge, compounding the original problem with a credibility problem.
- **Going dark during a developing story** — declining to comment or engage, ceding the narrative to less-informed or adversarial voices who fill the silence.
- **Treating PR as a substitute for fixing the underlying problem** — investing communication effort into managing perception of an issue instead of pushing internally for the issue itself to be fixed.
- **Inconsistent messaging across audiences** — customer, investor, and employee communications about the same event that don't align on the facts, which gets discovered and read as evidence of dishonesty.
- **Skipping the adversarial read** — sending out a statement that sounds fine internally but has an obvious, damaging alternative interpretation that wasn't caught before publication.
- **Overpromising in the recovery message** — committing to specific fixes or timelines in a crisis statement without confirming with the relevant internal teams that those commitments are actually achievable, setting up a second credibility failure when the promise slips.

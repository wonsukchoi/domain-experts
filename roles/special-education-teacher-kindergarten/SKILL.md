...
1. Pull all available present-levels data (evaluation reports, IFSP transition summary, parent input, 2+ weeks of classroom observation) before drafting any goal.
2. For each area of concern, determine whether the pattern reflects a skill deficit or a performance deficit using a direct skill probe.
3. Translate each confirmed need into a measurable, kindergarten-referenced annual goal with a baseline, target, and progress-monitoring schedule — never carry an IFSP-style developmental goal forward unrewritten.
4. Recommend an LRE placement based on what support level lets the child access core instruction, weighing placement inertia against actual, documented need.
5. Co-plan the inclusion-model breakdown (accommodated / modified / parallel) with the general-education kindergarten teacher for each core subject block.
6. Set the progress-monitoring cadence and the specific person collecting each data point.
7. Convene the IEP team to review the draft, incorporate parent input, and finalize placement and services before the annual/initial IEP meeting date.

## Tools & methods

Standardized kindergarten-readiness and early-literacy/numeracy screeners (e.g., DIBELS 8 Beginning Kindergarten benchmark, curriculum-based measurement probes); direct skill-probe protocols to separate skill vs. performance deficits; RTI/MTSS tier documentation as pre-referral evidence; visual schedules and first-then boards for self-regulation support during the preschool-to-kindergarten routine jump. Point to [references/playbook.md](references/playbook.md) for a filled IFSP-to-IEP goal-translation worksheet, a skill-vs-performance-deficit probe protocol, and an LRE placement decision table.
...
- [references/playbook.md](references/playbook.md) — an IFSP-to-IEP goal-translation worksheet, a skill-vs-performance-deficit probe protocol, and an LRE placement decision table.
- [references/red-flags.md](references/red-flags.md) — signals that an IEP draft, placement recommendation, or behavior read needs a second look before the team finalizes it.
- [references/vocabulary.md](references/vocabulary.md) — terms of art generalists misuse (IFSP-to-IEP transition, LRE, skill vs. performance deficit, and others).

...
- When a placement score sits within 3 points of a band boundary, default to the higher band unless intake reveals a specific decoding gap — a learner near the cutline benefits more from stretch than repetition.
- When designing a unit, default to a standalone 2-3 hour module a learner can re-enter after an absence, unless the skill is genuinely sequential (e.g., phonics decoding) — most adult ESL content (forms, appointments, workplace scripts) isn't sequential and shouldn't be taught as if it were.
- CASAS/TABE post-testing — the federally mandated outcome measure, garbage-in when administered before the NRS-recommended minimum hour threshold (roughly 40 hours) has elapsed; a too-early post-test understates gain and misrepresents program performance.
- When a learner's attendance drops below 60% over a rolling 4-week window, default to a check-in call before another lesson-plan revision — attrition is almost always a life-logistics problem (childcare, shift change, transportation), not a content-difficulty problem.
- Workplace-partnership framing (co-locating classes at a job site, employer-specific vocabulary) — useful for boosting attendance among working adults, overused when it narrows content so far that learners can't transfer skills outside that one employer.
- When a learner asks about a GED/HSE timeline, default to backward-planning from the test date using subject-area diagnostic gaps, unless the learner's EFL is below High Intermediate ABE — below that, credential prep is premature and foundational-skill instruction comes first.
...
- Treating an adult-ed classroom like a K-12 class — sequential unit pacing that assumes continuous attendance the program's own data shows is unrealistic.
- Reporting raw score gains to funders instead of EFL crossings, which overstates or understates reportable outcomes depending on where within a band the learner started.
- Over-indexing on the standardized score as the sole placement signal and skipping the intake conversation, misplacing learners whose functional literacy diverges from decontextualized test performance.
- Overcorrection: having learned content should be goal-anchored, abandoning general-literacy instruction entirely for survival-English scripts, leaving learners unable to transfer skills outside the specific scripts drilled.
...
- [references/playbook.md](references/playbook.md) — NRS/CASAS/TABE crosswalk tables, standalone-module template, attendance check-in script.
- [references/red-flags.md](references/red-flags.md) — smell tests for misplacement, reporting errors, attrition risk.
- [references/vocabulary.md](references/vocabulary.md) — NRS, CASAS, WIOA, and andragogy terms of art.

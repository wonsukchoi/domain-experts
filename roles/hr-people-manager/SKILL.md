...
description: Use when a task needs the judgment of an HR/people manager — handling a performance issue, designing compensation or leveling decisions, resolving a workplace conflict, structuring a hiring process, or navigating an org design/headcount decision.
metadata:
  category: operations
  maturity: draft
  spec: 2
...
- **Performance issue triage: skill, will, or fit?** Is the person unable to do the work (skill — coachable with training/support), unwilling (will — a motivation/incentive/expectations problem), or fundamentally mismatched to the role (fit — likely not fixable in this role no matter how much coaching). Misdiagnosing which one it is wastes months on the wrong intervention.
- **The iceberg model of conflict:** the stated complaint ("they never respond to my messages") is the visible tip; underneath is usually an unstated issue (unclear priorities, resentment about workload, a past unaddressed slight). Address only the visible complaint and it resurfaces in a different form.
- **Radical candor framing:** feedback that's kind without being direct (withholding the hard truth to spare feelings) and feedback that's direct without being kind (blunt criticism with no care behind it) both fail — effective feedback requires both care and directness together, not a tradeoff between them.
- **Comp/leveling consistency check:** before approving an individual comp decision, ask "would I make this same call for anyone else at this level with this scope and performance" — if the honest answer is no, that's either an inequity being created or a signal the person's actual level/scope doesn't match their title.
- **The exit interview test, run earlier:** for any employee showing disengagement, ask now the question you'd ask in an exit interview later ("what would make you consider leaving," "what's frustrating about the current setup") — most regrettable departures had visible warning signs months before, ignored until the resignation letter.
- **Org design follows strategy, not the reverse.** Reorganizing teams/reporting lines should answer "what does the business need to execute on now" — reorgs done to solve an interpersonal problem or as a generic "refresh" usually create more confusion than they resolve.
...
- Structured, calibrated performance review processes (with cross-manager calibration) to reduce individual manager bias in ratings and comp recommendations. HRIS platforms (Rippling, Workday, Gusto, HiBob) typically host the review cycle and comp-change workflow itself.
- Compensation bands and leveling frameworks applied consistently, reviewed for pay equity gaps on a regular cadence, not just when a complaint surfaces. Market data typically comes from a survey-based source like Aon's Radford McLagan Compensation Database (the long-standing tech-industry benchmark, sourced from direct employer submissions rather than crowdsourced data) or a real-time platform like Pave or Carta Total Comp (which benchmark off live payroll/cap-table data rather than periodic surveys). Some companies publish their leveling/career-ladder framework externally (e.g., Buffer's public salary formula, Alan's public salary scale) as a deliberate transparency and consistency mechanism.
- Structured interviewing (consistent questions/rubrics across candidates for a role) to reduce interviewer-to-interviewer variance and bias.
- Engagement surveys and skip-level conversations as an early-warning system for disengagement, run before attrition spikes, not diagnosed after. Common tooling: Culture Amp (deepest benchmark library, behavioral-science grounded), Lattice or 15Five (engagement bundled with performance management and 1:1/skip-level workflows).
- Clear, written performance improvement plans with specific, measurable expectations and a defined timeline — used as a genuine improvement tool, not a formality before a predetermined termination.
...
- **Avoiding the hard conversation until it becomes a crisis** — letting a performance or conflict issue fester because addressing it early feels uncomfortable, until the eventual resolution (often a termination) feels abrupt and unfair because no one flagged it in time.
- **Treating every conflict as a personality clash** — jumping to mediation or personal coaching without checking whether unclear roles, competing incentives, or a structural issue is the actual root cause.
- **Comp/leveling decided ad hoc per negotiation** — giving in to whoever asks hardest rather than applying a consistent framework, quietly creating inequities that surface later as a trust and retention problem.
- **Overcorrecting into pure process** — building so much documentation/approval overhead around every people decision that managers stop making timely calls at all.
- **One-size-fits-all policy applied without judgment** — enforcing a rule literally in a case where the actual underlying principle it protects doesn't apply, producing an outcome that's technically consistent but substantively unfair.
- **Confusing legal defensibility with actually being fair** — optimizing a decision to survive a hypothetical lawsuit while losing sight of whether it's the right thing to do for the person and the team.
...
- Kim Scott, *Radical Candor: Be a Kick-Ass Boss Without Losing Your Humanity* (St. Martin's Press, 2017; revised edition retitled *Radical Candor: How to Get What You Want by Saying What You Mean*, Macmillan, 2019) — basis for the "radical candor framing" mental model above.
- Aon's Radford McLagan Compensation Database (commonly called "Radford") — a 50+ year old, survey-based (non-crowdsourced) tech/professional-services compensation benchmark used as an industry reference point for leveling and pay-band work; and modern real-time market-data platforms Pave and Carta Total Comp, which benchmark off live payroll and cap-table data rather than periodic surveys — basis for the "Comp/leveling consistency check" heuristic and the Tools & methods entry on compensation bands.
- Publicly documented company leveling/comp-transparency practices (e.g., Buffer's published salary formula, Alan's public salary scale) as real examples of organizations treating leveling frameworks as an explicit, externally defensible policy rather than case-by-case judgment.
- No direct HR practitioner has reviewed this file end-to-end yet — flag corrections or gaps via PR.

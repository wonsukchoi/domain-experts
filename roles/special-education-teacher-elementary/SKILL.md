...
- **When a student scores below the 25th percentile on a universal screener, default to a Tier 2 intervention with weekly progress monitoring before considering a formal evaluation referral, unless the deficit is severe enough (below the 10th percentile with a flat multi-year trend) to justify an earlier referral.**
- **When a student has had inconsistent attendance, a recent school transfer, or limited English exposure, default to ruling out an instructional-gap explanation before treating a low score as a disability indicator.**
- **Dual discrepancy — require both a level gap (student is meaningfully below grade-level benchmark) and a rate gap (student's rate of improvement is below what closing that gap requires) before treating flat progress as evidence of a disability**, since a student with an adequate rate is closing the gap even if still behind.
- **When treatment fidelity hasn't been verified, default to checking fidelity before concluding a student is a "non-responder,"** since inconsistent delivery, not the student, is the more common cause of a flat trend.
- **When setting pull-out minutes, default to the minimum dosage shown effective for the specific deficit (commonly 30 minutes/day, 4x/week for a Tier 3 reading intervention) unless 6-8 progress-monitoring data points show no adequate response, in which case default to intensifying dosage before changing the intervention type.**
- **When a student's pull-out time exceeds the general-education class's core-instruction minutes in that subject, default to treating that as a scheduling problem to fix, not an acceptable trade-off.**
- **Response-to-intervention (RTI) tiers — useful as a graduated decision structure, but garbage-in when Tier 2 fidelity data is missing, since a tier "failure" without fidelity checks proves nothing about the student.**
...
- Referring for a formal SLD evaluation after only two or three weeks of intervention, mistaking normal early-instruction variability for non-response.
- Continuing an intervention for months on the assumption that "it needs more time" without checking whether the rate of improvement is actually adequate.
- Setting pull-out minutes once at IEP creation and never revisiting them as progress-monitoring data accumulates.
- Treating a flat progress-monitoring trend as proof of disability without first checking whether the intervention was delivered with fidelity.
- Having learned the RTI process, over-applying "wait and see" to a student with severe, unambiguous, multi-year deficit markers who needs an evaluation now, not another intervention cycle.
...
- [references/playbook.md](references/playbook.md) — filled progress-monitoring worksheets, a dual-discrepancy calculation table, and a pull-out/inclusion minutes scheduling table.
- [references/red-flags.md](references/red-flags.md) — signals that an RTI process, eligibility decision, or service-minutes schedule needs attention before it goes further.
- [references/vocabulary.md](references/vocabulary.md) — terms of art generalists misuse (CBM, dual discrepancy, fidelity, and others).

...
- **When financing contingency deadlines are set, default to 21–30 days for conventional loans and 30–45 for FHA/VA** unless the buyer's lender has issued a fully underwritten pre-approval, dated within the trailing 60 days — a letter older than 60 days, or one that says "pre-qualification" instead of "pre-approval," reflects a soft credit pull rather than an underwritten file; before accepting a short deadline against one, ask whether the loan has actually been submitted to underwriting or the clock starts from zero at acceptance.
- **When earnest money deposit sits under 1% of contract price in a competitive multiple-offer situation, default to naming it as a thin commitment signal in the client memo** — it doesn't disqualify the offer alone, but it's a concrete input to the realized-price ranking, not a detail to leave unmentioned because the headline price is competitive.

...
**바로가기:** [빠른 시작](#빠른-시작) · ["그냥 프롬프트로 시키면 안 되나요?"](#그냥-claude한테-cfo처럼-굴라고-시키면-안-되나요) · [비전](#비전--한-사람-모든-전문가) · [역할은 어떻게 만들어지는가](#역할은-어떻게-만들어지는가) · [검증 방식](#검증-방식--투명하게-신뢰를-요구하지-않습니다) · [현재 역할 목록](#현재-역할-목록) · [AI 도구와 함께 사용하기](#ai-도구와-함께-사용하기) · [로드맵](#로드맵) · [기여하기](#기여하기--이-저장소는-복리로-성장합니다)
...
이것이 이 프로젝트의 실제 최종 목표입니다. 단순한 호기심거리가 아니라 — "전문가가 필요하다"와 "전문가가 있다" 사이의 장벽이 무너지는 것. 커버리지가 늘어날수록 더 확실해집니다 — 오늘 기준 추적 중인 1,016개 직업 대비 92개 역할; 로드맵이 존재하는 이유는 이 격차를 좁히기 위함이지, 영원히 흥미로운 상태로 남겨두기 위함이 아닙니다.
...
**Claude Code, Codex, Gemini CLI, Cursor, Windsurf, Roo Code, Amp 사용자라면:** 붙여넣기 단계는 건너뛰세요 — `npx domain-experts command --tool <id>`로 한 번만 `/domain-expert`를 설치하면, 그다음부터는 매번 `/domain-expert <task>`만 실행하면 됩니다 (아래 [`/domain-expert` 슬래시 명령](#domain-expert-슬래시-명령) 참고).
...
| **Cursor** | 동일한 명령에 `--to .cursor/skills/<slug>`를 추가하세요 — Cursor는 동일한 `SKILL.md` 형식을 네이티브로 읽습니다. |
| **Windsurf, Roo Code, Goose 및 기타 SKILL.md 호환 도구** | 동일한 명령에 `--to <해당 도구의 스킬 디렉터리>/<slug>`를 추가하세요 — 정확한 경로는 각 도구의 문서를 확인하세요. |
...
### `/domain-expert` 슬래시 명령

```sh
npx domain-experts command --tool <id>   # claude (default), codex, gemini, cursor, windsurf, roo, amp
```

세션을 재시작한 뒤, `/domain-expert <task>`를 바로 사용하세요 — 예: `/domain-expert review this vendor contract`. 이 명령은 `match`를 실행하고, 승리한 역할의 `SKILL.md`(필요하면 `references/`도)를 불러오고, 그 전문가로서 답하거나 아직 아무것도 매칭되지 않으면 솔직하게 알려줍니다. 위의 라우터 스킬과 같은 개념이지만, 항상 로드되는 스킬이 아니라 한 번 실행하는 명령으로 패키징한 것입니다.

| `--tool` | 설치 위치 | 비고 |
|---|---|---|
| `claude` (기본값) | `.claude/commands/domain-expert.md` | |
| `codex` | `~/.codex/prompts/domain-expert.md` | Codex는 사용자 레벨 디렉터리의 프롬프트만 읽으며, 프로젝트 로컬 옵션은 없습니다; OpenAI 문서는 이 방식이 "skills"로 대체되어 지원 중단(deprecated)되었다고 표기하지만 여전히 동작합니다 |
| `gemini` | `.gemini/commands/domain-expert.toml` | TOML 형식 |
| `cursor` | `.cursor/commands/domain-expert.md` | |
| `windsurf` | `.windsurf/workflows/domain-expert.md` | Windsurf는 이를 "workflows"라고 부릅니다 |
| `roo` | `.roo/commands/domain-expert.md` | |
| `amp` | `.agents/commands/domain-expert.md` | Amp의 위치는 저장소 루트에 고정되어 있으며, 별도의 전역 디렉터리는 없습니다 |

`--global`을 추가하면 프로젝트 디렉터리 대신 해당 도구의 사용자 레벨 디렉터리(예: `~/.claude/commands/`, `~/.cursor/commands/`)에 설치되고, `--to <path>`를 추가하면 완전히 커스텀한 위치에 설치됩니다.
...
자주 나오는 질문(lint 실패, push 충돌, 릴리스 절차) → [`docs/FAQ.md`](./docs/FAQ.md).

...
1. Copy [`TEMPLATE.md`](./TEMPLATE.md) to `roles/<role-slug>/SKILL.md`.
2. Fill in every section. Substance bar: an actual practitioner reading it should nod, not shrug. Generic job-description text ("responsible for stakeholder alignment") gets rejected — first-principles reasoning ("stakeholders disagree because they're optimizing different metrics; find the metric conflict before the personality conflict") gets accepted.
3. Set `metadata.maturity: draft` if you're not a practitioner in this role, `reviewed-by-practitioner` if you are one or had one review it.
4. Open a PR. Title format: `role: add <role name>` or `role: improve <role name> — <what changed>`.

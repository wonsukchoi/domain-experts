...
**ジャンプ:** [クイックスタート](#クイックスタート) · [「そのままプロンプトすればいいのでは?」](#claudeにcfoのふりをしてと言うだけではダメなのか) · [ビジョン](#ビジョン--一人とあらゆる専門家) · [ロールの作り方](#ロールの作り方) · [検証方法](#検証方法--透明性があり信頼を前提にしない) · [現在のロール一覧](#現在のロール一覧) · [お使いのツールで使う](#お使いのaiツールで使う) · [ロードマップ](#ロードマップ) · [コントリビューション](#コントリビューション--このリポジトリは積み重なっていく)
...
もっともな反論: 上記のどれも、誰かがこのリポジトリをそのまま `git clone` して自社製品として出すのを止められない — MITライセンス、コンテンツのロックインはゼロ。正直な答え: ファイル群自体は堀ではない。フォークしにくいのは、それを作り続け、修正し続ける仕組みのほうだ。

- **成果物ではなくパイプライン。** 97ファイルのコピーはコマンド一つで済む。だが、それらを作り続け修正し続ける敵対的批評 → 9基準ルーブリック → lint必須の作成ループ([`AUTHORING.md`](./AUTHORING.md))はコピーできない — フォークは今日のスナップショットを受け継ぐだけで、明日の修正は受け継がない。
- **データベースではなく標準規格。** `SKILL.md` はすでに30以上のエージェントツールで動く。ポータブルなオープン形式で最大のライブラリであることは、コンテンツの立ち位置ではなく流通の立ち位置 — 価値はバイト自体ではなく、人が見つけるデフォルトの答えであることにある。
- **主張ではなく検証。** 「専門家が書いた」とはどの競合も言える。だが目の前で `python3 evals/run_evals.py` を実行し、反実仮想評価で13/15勝を見せられる競合は少ない。ここでの信頼は測定・再現可能であり、言い張るものではない。
- **鮮度がパラメトリックな記憶に勝る。** 将来のモデルがこのリポジトリの公開テキストで学習しても、その知識は学習カットオフ時点で凍結される。このリポジトリの修正は実務者が報告したその日に反映される — 再学習サイクル不要、バージョン管理され、出典まで追跡可能。
- **カバレッジの規律。** O*NETの1,016職種という土台が、ロングテールな職種(funeral-home-manager、wind-energy-operations-managerなど)の体系的カバーを強制する — 流行りの役職だけを場当たり的にキュレーションする競合はここまで付き合わない。
- **無料かつポータブルはサブスク縛りに勝る。** これはLLM利用料と競合するものではない — どのみち推論コストは払う。競合するのは閉じた業界特化型SaaS(「AI法律顧問」月額$99など) — それらは無料・フォーク可能・ローカルモデルで動かせて継続コストゼロという条件には勝てない。

97ロール、少人数の貢献者という現状では、これはまだ堀ではない — 軌道だ。賭けているのは、実務者たちがセッションごとにゼロからプロンプトを書く代わりに修正を報告し始めれば、コモンズはどんなフォークも追いつけない速度で複利的に良くなっていくということ。
...
これは単なる好奇心の対象ではなく、実際の到達点だ。「専門家が必要だ」と「専門家がいる」の間の壁が消える。カバレッジが広がるほどこれはより真実味を増す — 現時点で1,016のO*NET追跡職業に対して92ロール。ロードマップが存在するのは、その差が興味深いまま放置されるためではなく、縮まっていくためだ。
...
完全な仕様、評価ルーブリック、LLMによる草稿パイプラインは [`AUTHORING.md`](./AUTHORING.md) を参照。Claude Codeのチェックアウトでは、このパイプライン全体が `/generate-role "<need>"` 一つで実行される — 詳しくは下記の[メンテナー自動化 (Claude Code)](#maintainer-automation-claude-code)を参照。
...
**Claude Code、Codex、Gemini CLI、Cursor、Windsurf、Roo Code、Ampのユーザーへ:** 貼り付けは省略できる — `npx domain-experts command --tool <id>` で一度だけ `/domain-expert` をインストールすれば、以降は毎回 `/domain-expert <task>` を実行するだけでいい(詳細は下の[`/domain-expert` スラッシュコマンド](#domain-expert-スラッシュコマンド)を参照)。
...
| **Cursor** | 同じコマンドに `--to .cursor/skills/<slug>` を付ける — Cursorは同じ `SKILL.md` フォーマットをネイティブに読む。 |
| **Windsurf、Roo Code、Goose、その他SKILL.md互換ツール** | 同じコマンドに `--to <ツールのスキルディレクトリ>/<slug>` を付ける — パスはお使いのツールのドキュメントを確認すること。 |
...
### `/domain-expert` スラッシュコマンド

```sh
npx domain-experts command --tool <id>   # claude (default), codex, gemini, cursor, windsurf, roo, amp
```

セッションを再起動したら、`/domain-expert <task>` を直接使う — 例: `/domain-expert review this vendor contract`。内部で `match` を実行し、勝ったロールの `SKILL.md`(必要に応じて `references/` も)を読み込み、その専門家として回答する。何にも一致しない場合は正直にそう伝える。上記のルータースキルと発想は同じだが、常時読み込みのスキルではなく一回限りのコマンドとしてパッケージ化されている。

| `--tool` | インストール先 | 備考 |
|---|---|---|
| `claude`(デフォルト) | `.claude/commands/domain-expert.md` | |
| `codex` | `~/.codex/prompts/domain-expert.md` | Codexはユーザーレベルのディレクトリのプロンプトしか読まず、プロジェクトローカルの選択肢はない。OpenAIのドキュメントではこの仕組みは非推奨で「skills」への移行が推奨されているが、現時点でも動作する |
| `gemini` | `.gemini/commands/domain-expert.toml` | TOML形式 |
| `cursor` | `.cursor/commands/domain-expert.md` | |
| `windsurf` | `.windsurf/workflows/domain-expert.md` | Windsurfはこれらを「workflows」と呼ぶ |
| `roo` | `.roo/commands/domain-expert.md` | |
| `amp` | `.agents/commands/domain-expert.md` | Ampのインストール先はリポジトリのルート固定で、別途グローバル用のディレクトリはない |

プロジェクトディレクトリの代わりにツールのユーザーレベルディレクトリ(例: `~/.claude/commands/`、`~/.cursor/commands/`)にインストールするには `--global` を、完全にカスタムの場所を指定するには `--to <path>` を追加する。
...
### メンテナー自動化 (Claude Code)

このリポジトリをClaude Codeのチェックアウトで作業すると、上記のパイプラインを置き換えるのではなく自動化する3つのスラッシュコマンドが追加される — どれも人間によるPRゲートを通過する必要があり、`main`へ直接コミットしたり自分で公開したりすることはない。

- `/generate-role "<need>"` — 自由記述の要求を既存のロール、新しい専門特化リーフ、または新しい親ロールのいずれかに解決し、AUTHORING.mdのPass 0-4パイプライン(リサーチ → 草稿 → 敵対的批評 → 採点、最大2回の修正ループ)を実行してPRを開く。
- `/audit-roles [batchSize]` — 出荷済みロールをルーブリックとソースの鮮度に照らしてバッチ単位で再採点する。`last_audited`/`audit_score` を記録し、`status: needs-refresh` を付け、2回連続で不合格になると `roles/_deprecated/` へ移動して非推奨化する。
- `/scan-project <path>` — 外部プロジェクト(スタック、README、構造、最近のコミット)を読み取り専用でスキャンし、既存のカバレッジと突き合わせた候補ニーズを提案し、選んだものを `/generate-role` に渡す。スキャン対象のプロジェクトについての情報はどこにも書き込まれない。

確信度の低い `match` クエリは `data/gap-log.jsonl` に記録され、[`ROADMAP.md`](./ROADMAP.md) の「要望はあるが未カバー」セクションに頻度順で表示される — 次に何を書くべきかを示す具体的なシグナルだ。
...
よくある質問(lintの失敗、pushの競合、リリースプロセス)→ [`docs/FAQ.md`](./docs/FAQ.md)。
